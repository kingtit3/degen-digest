...
from utils.advanced_logging import get_logger, log_system_health, log_performance_metrics
...
class SystemMonitor:
        ...
        try:
            with Session(engine) as session:
                ...
                recent_tweets = session.exec(
                    select(func.count()).select_from(Tweet).where(Tweet.created_at >= cutoff)
                ).one()
                
                recent_reddit = session.exec(
                    select(func.count()).select_from(RedditPost).where(RedditPost.created_at >= cutoff)
                ).one()
                ...
                if latest_tweet:
                    hours_since_latest = (datetime.now(timezone.utc) - latest_tweet.created_at).total_seconds() / 3600
                ...
                health_metrics = {
                    ...
                    "data_freshness": {
                        ...
                        "hours_since_latest": hours_since_latest if latest_tweet else None
                    },
                    "llm_usage": {
    ...
    def check_cloud_function_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_metrics = {
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "function_status": "healthy" if response.status_code == 200 else "unhealthy",
        ...
        try:
            with Session(engine) as session:
                ...
                no_engagement_tweets = session.exec(
                    ...
                    .where(Tweet.like_count == 0, Tweet.retweet_count == 0, Tweet.reply_count == 0)
                ).one()
                ...
                quality_metrics = {
                    ...
                    "data_quality_score": 100 - (len(duplicate_tweets) * 5) - (malformed_tweets * 10)
                }
    ...
    def run_comprehensive_monitoring(self) -> Dict[str, Any]:
        ...
        log_system_health("overall", overall_status, {
            "alerts_count": len(alerts),
            "critical_alerts": len([a for a in alerts if a["level"] == "critical"]),
            "warning_alerts": len([a for a in alerts if a["level"] == "warning"])
        })
        
        logger.info(f"Monitoring complete. Status: {overall_status}, Alerts: {len(alerts)}")
