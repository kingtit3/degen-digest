...
class ComprehensiveTester:
    ...
    async def test_enhanced_data_pipeline(self):
        ...
        try:
            ...
            success = (
                total_items > 0
                and viral_predictions > 0
                and data_sources > 0
                and pipeline_time < 300  # Should complete within 5 minutes
            )
            ...
            self.test_results["enhanced_data_pipeline"] = {
                ...
                "details": {
                    "sources_with_data": [
                        k
                        for k, v in pipeline.data_sources.items()
                        if v.get("count", 0) > 0
                    ],
                    "sources_with_errors": [
                        k for k, v in pipeline.data_sources.items() if v.get("error")
                    ],
                },
            }

            print(
                f"✅ Enhanced Data Pipeline: {total_items} items, {viral_predictions} predictions, {pipeline_time:.2f}s"
            )
    ...
    async def test_data_quality_monitoring(self):
        ...
        try:
            ...
            success = (
                ...
                and "test_source" in quality_summary["source_status"]
            )
            ...
            print(
                f"✅ Data Quality Monitoring: Score {quality_report['overall_score']:.3f}, {len(quality_report['alerts'])} alerts"
            )
    ...
    async def test_enhanced_viral_predictor(self):
        ...
        try:
            ...
            success = (
                len(predictions) > 0
                and all("score" in pred for pred in predictions)
                and all(0 <= pred["score"] <= 1 for pred in predictions)
            )
    ...
    async def test_automated_retraining(self):
        ...
        try:
            ...
            success = (
                "total_retrains" in stats
                and "current_performance" in stats
                and "performance_trend" in stats
            )
            ...
            print(
                f"✅ Automated Retraining: System initialized, {stats.get('total_retrains', 0)} retrains"
            )
    ...
    async def test_integration(self):
        ...
        try:
            ...
            success = (
                len(pipeline.processed_data) > 0
                and len(quality_reports) > 0
            ...
            print(
                f"✅ Integration: {len(pipeline.processed_data)} items, {len(predictions)} predictions"
            )
