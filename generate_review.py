...
def build_review_md() -> str:
    ...
    # --------------------------------------------------------------------
    md_lines: list[str] = []
    md_lines.append(f"# 📈 Degen Digest – Comprehensive Market Review\n")
    md_lines.append(f"**Date:** {date.today().strftime('%B %d, %Y')}\n")
    md_lines.append(f"**Report Type:** Daily Market Analysis\n")
    ...
    engagement_scores = [item.get('_engagement_score', 0) for item in processed if item.get('_engagement_score')]
    if engagement_scores:
        md_lines.append(f"- Average Engagement Score: {sum(engagement_scores)/len(engagement_scores):.2f}\n")
    ...
    if top_terms:
        md_lines.append("| Term | Hourly Change | Current Count | Previous Hour | Acceleration Ratio |\n")
        md_lines.append("|------|---------------|---------------|---------------|-------------------|\n")
        for ratio, term, cnt, prev_cnt in top_terms:
            change = cnt - prev_cnt
            change_str = f"+{change}" if change > 0 else str(change)
            md_lines.append(f"| {term.upper()} | {change_str} | {cnt} | {prev_cnt} | {ratio:.1f}× |\n")
    ...
    md_lines.append("**Note:** These stories show high engagement potential but haven't yet reached viral status.\n\n")
    ...
    positive_items = [item for item in processed if item.get('_sentiment_score', 0) > 0.1]
    negative_items = [item for item in processed if item.get('_sentiment_score', 0) < -0.1]
    neutral_items = [item for item in processed if abs(item.get('_sentiment_score', 0)) <= 0.1]
    
    md_lines.append(f"**Overall Sentiment Distribution:**\n")
    md_lines.append(f"- Positive Sentiment: {len(positive_items)} stories ({len(positive_items)/len(processed)*100:.1f}%)\n")
    md_lines.append(f"- Neutral Sentiment: {len(neutral_items)} stories ({len(neutral_items)/len(processed)*100:.1f}%)\n")
    md_lines.append(f"- Negative Sentiment: {len(negative_items)} stories ({len(negative_items)/len(processed)*100:.1f}%)\n\n")
    
    # Top positive and negative stories
    if positive_items:
        md_lines.append("**Top Positive Stories:**\n")
        for item in positive_items[:5]:
            md_lines.append(f"- {item['headline']} (Sentiment: {item.get('_sentiment_score', 0):.2f})\n")
        md_lines.append("\n")
    ...
    md_lines.append("**Investment Risk Warning:** This report is for informational purposes only and does not constitute financial advice. "
                   "Cryptocurrency investments carry significant risk, including the potential for total loss of capital.\n\n")
    md_lines.append("**Data Accuracy:** While we strive for accuracy, market data and social media content can be volatile and subject to rapid change.\n\n")
    md_lines.append("**Not Financial Advice:** Always conduct your own research and consider consulting with qualified financial advisors before making investment decisions.\n\n")
    md_lines.append(f"**Report Generated:** {date.today().strftime('%B %d, %Y at %H:%M UTC')}\n")
    md_lines.append("**Data Sources:** Twitter, Reddit, Telegram, News APIs, CoinGecko\n")
