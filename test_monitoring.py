...
from storage.db import Tweet, engine
...
def test_performance_logging():
    ...
    log_system_health(
        "test_component",
        "healthy",
        {"response_time_ms": 150, "error_rate": 0.01, "throughput": 1000},
    )
    ...
    log_data_quality_issue(
        "duplicate_data",
        "medium",
        {"duplicate_count": 5, "total_records": 1000, "duplicate_percentage": 0.5},
    )
    ...
    log_security_event(
        "api_rate_limit",
        "warning",
        {"api_endpoint": "/api/data", "rate_limit": 100, "current_requests": 95},
    )

    # Test business event logging
    log_business_event(
        "digest_generated",
        {"digest_id": "test_123", "content_items": 50, "generation_time": 30.5},
    )
...
def test_database_monitoring():
    ...
    try:
        ...
        print("📊 Database Health Results:")
        print(f"  - Overall status: {db_health.get('overall_status', 'unknown')}")
        print(
            f"  - Tweet count: {db_health.get('data_counts', {}).get('total_tweets', 0)}"
        )
        print(
            f"  - Reddit count: {db_health.get('data_counts', {}).get('total_reddit_posts', 0)}"
        )
        print(
            f"  - Data freshness: {db_health.get('data_freshness', {}).get('status', 'unknown')}"
        )
...
def test_alert_generation():
    ...
    try:
        ...
        test_metrics = {
            ...
            "database": {"overall_status": "critical"},  # Should trigger critical alert
            "data_quality": {"data_quality_score": 65},  # Should trigger warning
        }

        # Generate alerts
        alerts = system_monitor.generate_alerts(test_metrics)

        print("🚨 Alert Generation Results:")
...
def test_log_file_analysis():
    ...
    if log_file.exists():
        ...
        print(
            f"  - Total log entries: {info_count + debug_count + warning_count + error_count}"
        )
