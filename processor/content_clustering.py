...
class ContentClusterer:
    ...
    def cluster_content(self, items: List[Dict], method: str = 'dbscan') -> Dict:
        ...
        for label, cluster_items in clusters.items():
            ...
            tickers = []
            for item in cluster_items:
                tickers.extend(extract_tickers(f"{item.get('text', '')} {item.get('title', '')}"))
            
            # Calculate engagement stats
            engagement_scores = [item.get('_engagement_score', 0) for item in cluster_items]
            ...
            cluster_stats[label] = {
                ...
                'sources': Counter([item.get('_source', 'unknown') for item in cluster_items]),
                'items': cluster_items
            }
    ...
    def find_similar_content(self, target_item: Dict, items: List[Dict], top_k: int = 5) -> List[Tuple[Dict, float]]:
    ...
    def get_cluster_summary(self) -> List[Dict]:
        ...
        for cluster_id, stats in self.clusters.items():
            summary = {
                ...
                'top_sources': [source for source, count in stats['sources'].most_common(3)],
                'sample_titles': [
