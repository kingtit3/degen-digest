...
class EnhancedViralPredictor:
    ...
    def _extract_author_features(self, item: Dict) -> Dict:
        ...
        account_age = item.get('userAccountAgeDays', 0) or item.get('author_account_age_days', 0)
    ...
    def _extract_content_quality_features(self, item: Dict) -> Dict:
        ...
        return {
            ...
            'content_complexity': len(set(text.split())) / len(text.split()) if text.split() else 0,
        }
    ...
    def _extract_market_context_features(self, item: Dict) -> Dict:
        ...
        return {
            'mentions_bitcoin': 1 if any(word in text_lower for word in ['bitcoin', 'btc', '$btc']) else 0,
            'mentions_ethereum': 1 if any(word in text_lower for word in ['ethereum', 'eth', '$eth']) else 0,
            'mentions_defi': 1 if any(word in text_lower for word in ['defi', 'yield', 'apy']) else 0,
            'mentions_nft': 1 if any(word in text_lower for word in ['nft', 'opensea']) else 0,
            'mentions_meme': 1 if any(word in text_lower for word in ['meme', 'dog', 'cat']) else 0,
            'mentions_airdrop': 1 if any(word in text_lower for word in ['airdrop', 'claim']) else 0,
            'mentions_scam': 1 if any(word in text_lower for word in ['rug', 'scam', 'honeypot']) else 0,
            'mentions_pump': 1 if any(word in text_lower for word in ['pump', 'moon', 'bull']) else 0,
        }
    ...
    def _extract_sentiment_features(self, text: str) -> Dict:
        ...
        crypto_positive = ['moon', 'pump', 'bull', 'buy', 'long', 'hodl', 'diamond', 'rocket', '🚀', '💎', 'lambo']
        crypto_negative = ['dump', 'bear', 'sell', 'short', 'rug', 'scam', 'dead', '💀', '📉', 'rekt']
        ...
        return {
            ...
            'crypto_sentiment': (crypto_pos_count - crypto_neg_count) / max(len(words), 1),
    ...
    def _extract_topic_features(self, text: str) -> Dict:
        ...
        topics = {
            'topic_bitcoin': 1 if any(word in text_lower for word in ['bitcoin', 'btc', '$btc']) else 0,
            'topic_ethereum': 1 if any(word in text_lower for word in ['ethereum', 'eth', '$eth']) else 0,
            'topic_defi': 1 if any(word in text_lower for word in ['defi', 'yield', 'apy', 'liquidity']) else 0,
            'topic_nft': 1 if any(word in text_lower for word in ['nft', 'opensea', 'floor']) else 0,
            'topic_meme': 1 if any(word in text_lower for word in ['meme', 'dog', 'cat', 'pepe']) else 0,
            'topic_airdrop': 1 if any(word in text_lower for word in ['airdrop', 'claim', 'free']) else 0,
            'topic_scam': 1 if any(word in text_lower for word in ['rug', 'scam', 'honeypot']) else 0,
            'topic_pump': 1 if any(word in text_lower for word in ['pump', 'moon', 'bull']) else 0,
            'topic_trading': 1 if any(word in text_lower for word in ['trade', 'chart', 'technical', 'analysis']) else 0,
            'topic_news': 1 if any(word in text_lower for word in ['news', 'announcement', 'update']) else 0,
        }
    ...
    def prepare_training_data(self, items: List[Dict]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for item in items:
            ...
            try:
                ...
                target = item.get('_engagement_score', 0) or item.get('viral_coefficient', 0)
    ...
    def train(self, items: List[Dict], test_size: float = 0.2):
        ...
        # Feature selection
        X_train_selected = self.feature_selectors['kbest'].fit_transform(X_train_scaled, y_train)
        ...
        for name, model in self.models.items():
            try:
                ...
                logger.info(f"{name} model - MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}")
        ...
        try:
            ...
            logger.info(f"Ensemble model - MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}")
    ...
    def get_feature_importance(self, model_name: str = 'rf') -> Dict:
        ...
        # Get feature names
        feature_names = [f"feature_{i}" for i in range(len(self.feature_importance[model_name]))]
