...
class ComprehensiveDataCleanup:
    ...
    def deduplicate_data_source(self, filename: str, source_name: str) -> Dict[str, Any]:
        ...
        if not file_path.exists():
            ...
            return {"items": [], "duplicates_removed": 0, "original_count": 0, "final_count": 0}
        ...
        try:
            ...
            deduplicated_file = self.deduplicated_dir / f"{source_name.lower()}_deduplicated.json"
            ...
            logger.info(f"✅ {source_name}: {duplicates_removed} duplicates removed ({original_count} → {len(unique_items)})")
    ...
    def run_deduplication(self):
        ...
        for filename, source_name in sources:
            results[source_name.lower()] = self.deduplicate_data_source(filename, source_name)
    ...
    def create_clean_consolidated_data(self, results: Dict[str, Any]):
        ...
        logger.info(f"✅ Clean consolidated data saved: {total_items} items, {total_duplicates} duplicates removed")
        
        return consolidated_data
    
    def generate_cleanup_report(self, results: Dict[str, Any], cache_cleared: int, backup_path: Path):
