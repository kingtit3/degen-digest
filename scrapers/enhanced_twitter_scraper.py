...
class EnhancedTwitterScraper:
    ...
    def _calculate_engagement_velocity(self, tweet_data: Dict) -> float:
        ...
        created_at = datetime.fromisoformat(tweet_data.get('createdAt', '').replace('Z', '+00:00'))
    ...
    def _calculate_viral_coefficient(self, tweet_data: Dict) -> float:
        ...
        viral_actions = tweet_data.get('retweetCount', 0) + tweet_data.get('quoteCount', 0)
    ...
    def _calculate_influence_score(self, tweet_data: Dict) -> float:
        ...
        hashtag_count = len([w for w in tweet_data.get('text', '').split() if w.startswith('#')])
        url_count = len([w for w in tweet_data.get('text', '').split() if 'http' in w])
        
        content_factor = min(1.0, (text_length / 280) * (1 + hashtag_count * 0.1) * (1 + url_count * 0.2))
    ...
    def _analyze_content(self, text: str) -> Dict[str, Any]:
        ...
        positive_words = ['moon', 'pump', 'bull', 'buy', 'long', 'hodl', 'diamond', 'rocket', '🚀', '💎']
        negative_words = ['dump', 'bear', 'sell', 'short', 'rug', 'scam', 'dead', '💀', '📉']
        
        positive_count = sum(1 for word in words if word.lower() in positive_words)
        negative_count = sum(1 for word in words if word.lower() in negative_words)
        
        analysis['sentiment_score'] = (positive_count - negative_count) / max(len(words), 1)
    ...
    async def scrape_enhanced_tweets(self, 
                                   accounts: List[str], 
                                   search_terms: List[str],
                                   max_tweets: int = 500,
                                   min_likes: int = 10,
                                   min_retweets: int = 5) -> List[EnhancedTweet]:
        ...
        for tweet_data in raw_tweets:
            try:
                ...
                logger.error(f"Failed to process tweet {tweet_data.get('id', 'unknown')}: {e}")
                continue
    ...
    def save_enhanced_data(self, tweets: List[EnhancedTweet]):
        ...
        metadata = {
            ...
            'avg_engagement_velocity': sum(t.engagement_velocity for t in tweets) / len(tweets) if tweets else 0,
            'avg_viral_coefficient': sum(t.viral_coefficient for t in tweets) / len(tweets) if tweets else 0,
            'avg_influence_score': sum(t.influence_score for t in tweets) / len(tweets) if tweets else 0,
        }
...
async def main():
    ...
    # Print summary
    if enhanced_tweets:
        avg_velocity = sum(t.engagement_velocity for t in enhanced_tweets) / len(enhanced_tweets)
        avg_viral_coef = sum(t.viral_coefficient for t in enhanced_tweets) / len(enhanced_tweets)
        avg_influence = sum(t.influence_score for t in enhanced_tweets) / len(enhanced_tweets)
        
        logger.info(f"Enhanced Twitter scrape completed:")
