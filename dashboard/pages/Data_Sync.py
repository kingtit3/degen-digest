...
# Page config
st.set_page_config(
    page_title="Data Sync - Degen Digest",
    page_icon="🔄",
    layout="wide"
)
...
if st.sidebar.button("🔄 Run Data Merger"):
    with st.spinner("Running data merger..."):
        import subprocess
        try:
            result = subprocess.run(["python", "scripts/merge_local_cloud_data.py"], 
...
if st.sidebar.button("☁️ Upload to Cloud"):
    with st.spinner("Uploading to cloud..."):
        import subprocess
        try:
            result = subprocess.run(["python", "scripts/cloud_storage_sync.py", "--direction", "upload"], 
...
if st.sidebar.button("⬇️ Download from Cloud"):
    with st.spinner("Downloading from cloud..."):
        import subprocess
        try:
            result = subprocess.run(["python", "scripts/cloud_storage_sync.py", "--direction", "download"], 
                                  capture_output=True, text=True, timeout=120)
            if result.returncode == 0:
...
# Main content tabs
tab1, tab2, tab3, tab4 = st.tabs(["📊 Data Overview", "🔄 Sync Status", "📈 Analytics", "🔧 Manual Sync"])
...
with tab1:
    ...
    if timeline_data:
        ...
        fig = px.line(daily_counts, x='date', y='count', color='source',
    ...
    fig = px.pie(values=list(source_counts.values()), names=list(source_counts.keys()),
...
with tab2:
    ...
    important_files = [
        ...
        "newsapi_raw.json", "coingecko_raw.json", "consolidated_data.json",
        "dashboard_processed_data.json", "degen_digest.db"
    ]
    ...
    for filename in important_files:
        ...
        if file_path.exists():
            ...
            file_status.append({
                ...
                'Last Modified': datetime.fromtimestamp(file_path.stat().st_mtime).strftime('%Y-%m-%d %H:%M')
            })
        else:
            file_status.append({
                'File': filename,
...
with tab3:
    ...
    if dashboard_data:
        ...
        if viral_scores:
            ...
            with col1:
                fig = px.histogram(x=viral_scores, title="Viral Score Distribution",
                                 labels={'x': 'Viral Score', 'y': 'Count'})
                st.plotly_chart(fig, use_container_width=True)
            
            with col2:
                st.metric("Average Viral Score", f"{sum(viral_scores)/len(viral_scores):.2f}")
        ...
        top_viral = sorted(dashboard_data, key=lambda x: x.get('viral_score', 0), reverse=True)[:10]
