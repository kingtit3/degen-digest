...
def main():
    ...
    with st.sidebar:
        ...
        sources = st.multiselect(
            "Select sources", ["Twitter", "Reddit"], default=["Twitter", "Reddit"]
        )
    ...
    if data:
        ...
        with col2:
            st.metric(
                ...
                f"{df.groupby('date')['engagement'].sum().mean():.0f}",
            )
        with col3:
            st.metric(
                "Peak Daily Engagement",
                f"{df.groupby('date')['engagement'].sum().max():,}",
            )
        with col4:
...
def show_sentiment_analysis(start_date, end_date, sources):
                    ...
                    data.append(
                        {
                            ...
                            "text": text,
                            "source": "Reddit",
                            "engagement": (post.score or 0)
                            + (post.numComments or 0) * 2,
                        }
                    )
    ...
    if data:
        ...
        with col4:
            st.metric(
                "Neutral Posts",
                f"{len(df[(df['sentiment'] >= -0.1) & (df['sentiment'] <= 0.1)]):,}",
            )
...
def show_source_performance(start_date, end_date, sources):
    ...
    if data:
        ...
        daily_performance = (
            df.groupby(["date", "source"])["engagement"].sum().reset_index()
        )
...
def show_market_insights(start_date, end_date, sources):
    ...
    if data:
        ...
        for i, row in top_content.iterrows():
            with st.expander(
                f"#{i+1} - Engagement: {row['engagement']:.0f} ({row['source']})"
            ):
        ...
        with col2:
            ...
            neutral_posts = len(
                df[(df["sentiment"] >= -0.1) & (df["sentiment"] <= 0.1)]
            )
        ...
        weekly_trends = (
            df.groupby("week")
            .agg({"engagement": "sum", "sentiment": "mean", "text": "count"})
            .reset_index()
        )
