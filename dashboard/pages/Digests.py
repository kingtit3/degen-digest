...
def get_latest_digest_files():
    ...
    if not output_dir:
        st.error(
            f"Could not find output directory. Tried: {[str(p) for p in possible_paths]}"
        )
        return {"pdf": None, "md": None, "current_md": None}

    # Find the most recent digest files
    digest_files = {"pdf": None, "md": None, "current_md": None}
...
def main():
    ...
    with st.expander("🔧 Debug Information", expanded=False):
        ...
        st.write(f"Available digest files: {digest_files}")
        st.write(f"Output directory exists: {Path('output').exists()}")
        if Path("output").exists():
            st.write(f"Files in output: {list(Path('output').glob('digest*'))}")
    ...
    if not has_current:
        st.warning(
            "⚠️ No current digest found. Run the data processing pipeline to generate a new digest."
        )
    ...
    with tab1:
        ...
        if digest_files["pdf"]:
            ...
            st.info(
                "No PDF digest available. The digest may be in Markdown format only."
            )
...
def show_archive():
    ...
    # Use the same path resolution as get_latest_digest_files
    possible_paths = [
        Path("output"),
        Path("../output"),
        Path("../../output"),
    ...
    if not output_dir:
        st.error(
            f"Could not find output directory for archive. Tried: {[str(p) for p in possible_paths]}"
        )
        return
    pdf_files = sorted(
    ...
    md_files = sorted(
        output_dir.glob("digest-*.md"), key=lambda p: p.stat().st_mtime, reverse=True
    )
    ...
    if pdf_files or md_files:
        ...
        with col1:
            ...
            for pdf in pdf_files[:5]:  # Show last 5
                file_time = datetime.fromtimestamp(pdf.stat().st_mtime).strftime(
                    "%Y-%m-%d %H:%M"
                )
        ...
        with col2:
            ...
            for md in md_files[:5]:  # Show last 5
                file_time = datetime.fromtimestamp(md.stat().st_mtime).strftime(
                    "%Y-%m-%d %H:%M"
                )
