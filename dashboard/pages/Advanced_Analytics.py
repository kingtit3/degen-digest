            ...
            items.append(
                {
                    ...
                    "published": tweet.published.isoformat()
                    if tweet.published
                    else None,
                    ...
                    "link": f"https://twitter.com/user/status/{tweet.tweet_id}"
                    if tweet.tweet_id
                    else None,
                }
            )
...
filtered_items = [
    ...
    and datetime.fromisoformat(item["published"].replace("Z", "+00:00")) > cutoff_date
]
...
with col1:
    ...
    daily_engagement = (
        df.groupby("date")
        .agg({"_engagement_score": ["mean", "sum", "count"]})
        .reset_index()
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=daily_engagement["avg_engagement"],
            mode="lines+markers",
            name="Avg Engagement",
        ),
        row=1,
...
with col2:
    ...
    fig = px.pie(
        values=source_counts.values, names=source_counts.index, title="Posts by Source"
    )
    st.plotly_chart(fig, use_container_width=True)
...
df["predicted_score"] = [
    item.get("_predicted_viral_score", 0) for item in filtered_items
]
...
with col2:
    ...
    top_predicted = sorted(
        filtered_items, key=lambda x: x.get("_predicted_viral_score", 0), reverse=True
    )[:10]

    for i, item in enumerate(top_predicted[:5]):
        with st.expander(f"#{i+1} - {item.get('title', 'No title')[:50]}..."):
            st.write(
                f"**Predicted Score:** {item.get('_predicted_viral_score', 0):.2f}"
            )
...
if search_text:
    ...
    if similar_items:
        ...
        for item, similarity in similar_items:
            with st.expander(
                f"Similarity: {similarity:.3f} - {item.get('title', 'No title')[:50]}..."
            ):
                st.write(f"**Source:** {item.get('_source', 'unknown')}")
...
with col4:
    st.metric(
        "Prediction Accuracy",
        f"{np.corrcoef(df['_engagement_score'], df['predicted_score'])[0,1]:.3f}",
    )
