...
def load_digest_files():
    ...
    possible_paths = [
        ...
        Path(os.path.join(os.path.dirname(__file__), "../../output")),  # Absolute from this file
    ...
    if not output_dir:
        st.error(f"Could not find output directory. Tried paths: {[str(p) for p in possible_paths]}")
        return []
...
def display_digest_archive():
    ...
    with col1:
        search_term = st.text_input("🔍 Search digests", placeholder="Search for keywords...")
    ...
    with col3:
        sort_by = st.selectbox(
            "📊 Sort by",
            ["Date (Newest)", "Date (Oldest)", "File Size"]
        )
    ...
    if search_term:
        filtered_digests = [
            d for d in filtered_digests 
            if search_term.lower() in d['content'].lower()
        ]
    ...
    for digest in filtered_digests:
        ...
        summary = ""
        for line in lines:
            if line.startswith('## 📋 Executive Summary') or line.startswith('## 🎯 Key Takeaways'):
    ...
    if filtered_digests:
        selected_digest = st.selectbox(
            ...
            options=[f"{d['date'].strftime('%Y-%m-%d')} - {d['filename']}" for d in filtered_digests],
            index=0
        )
        ...
        for digest in filtered_digests:
            if (digest['date'].strftime('%Y-%m-%d') == selected_date and 
                digest['filename'] == selected_filename):
                
                # Display digest content
                st.markdown("---")
                st.markdown(f"### 📄 {digest['date'].strftime('%B %d, %Y')} - {digest['filename']}")
                ...
                with st.expander("📄 Click to expand full digest content", expanded=True):
