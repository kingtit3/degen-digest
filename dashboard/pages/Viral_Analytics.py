...
def main():
    st.set_page_config(
        page_title="Viral Analytics - Degen Digest",
        page_icon="🚀",
        layout="wide"
    )
    ...
    if not data:
        st.warning("No viral analytics data available. Run the enhanced data pipeline first.")
...
def filter_data(data, data_source, time_range, viral_threshold):
    ...
    if data_source != "All Sources":
        processed_data = [item for item in processed_data if item.get('source') == data_source.lower()]
        viral_predictions = [pred for pred in viral_predictions if pred.get('source') == data_source.lower()]
    ...
    if time_range != "All Time":
        ...
        processed_data = [
            ...
            if parse_timestamp(item.get('processed_at', item.get('created_at', ''))) > cutoff_time
        ]
        viral_predictions = [
            pred for pred in viral_predictions 
...
def display_quick_stats(data):
    ...
    high_viral_count = len([p for p in viral_predictions if p['prediction']['score'] > 0.7])
...
def display_predictions_analysis(data):
    ...
    top_predictions = df.nlargest(10, 'viral_score')[['source', 'viral_score', 'confidence', 'content_preview']]
...
def display_trends_analysis(data):
    ...
    fig = px.bar(
        ...
        labels={'avg_engagement_velocity': 'Avg Engagement Velocity', 'source': 'Source'}
    )
    st.plotly_chart(fig, use_container_width=True)
    ...
    if all_categories:
        ...
        category_counts = category_df.groupby('category')['count'].sum().sort_values(ascending=False)
...
def display_top_viral_content(data):
    ...
    # Sort by viral score
    sorted_predictions = sorted(viral_predictions, key=lambda x: x['prediction']['score'], reverse=True)
    
    # Display top 10
    st.subheader("Top 10 Viral Content Items")
    
    for i, pred in enumerate(sorted_predictions[:10], 1):
        with st.expander(f"#{i} - Score: {pred['prediction']['score']:.3f} | {pred['source'].title()}"):
...
def display_real_time_monitoring():
    ...
    alerts = [
        {"time": "2 min ago", "type": "High Viral Score", "content": "New Bitcoin tweet scored 0.89"},
        {"time": "5 min ago", "type": "Trending Topic", "content": "DeFi mentions increased 150%"},
        {"time": "8 min ago", "type": "Model Update", "content": "Model retrained with new data"}
    ]
