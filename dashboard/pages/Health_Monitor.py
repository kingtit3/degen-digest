...
def get_database_metrics():
    ...
    try:
        with Session(engine) as session:
            ...
            if latest_tweet:
                tweet_freshness = (datetime.now(timezone.utc) - latest_tweet.created_at).total_seconds() / 3600
            
            if latest_reddit:
                reddit_freshness = (datetime.now(timezone.utc) - latest_reddit.created_at).total_seconds() / 3600
...
def display_database_health(metrics):
    ...
    with col4:
        recent_total = db_metrics.get('recent_tweets_24h', 0) + db_metrics.get('recent_reddit_24h', 0)
    ...
    fig = px.bar(df_activity, x="Source", y="Count (24h)", 
                 title="Data Collection Activity (Last 24 Hours)",
                 color="Source", color_discrete_map={"Tweets": "blue", "Reddit Posts": "orange"})
...
def display_api_health(metrics):
    ...
    st.dataframe(df_api.style.applymap(color_status, subset=["Status"]), use_container_width=True)
...
def display_data_quality(metrics):
    ...
    fig = px.bar(df_quality, x="Metric", y="Value", 
...
def main():
    st.set_page_config(
        page_title="Health Monitor - Degen Digest",
        page_icon="🏥",
        layout="wide"
    )
