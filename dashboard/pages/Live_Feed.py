...
def main():
    ...
    with st.sidebar:
        ...
        sort_by = st.selectbox(
            "Sort by",
            ["Engagement Score", "Date (Newest)", "Date (Oldest)", "Source", "Content Length"]
        )
    ...
    with col1:
        ...
        with st.spinner("Loading data..."):
            ...
            if "Twitter" in sources:
                with Session(engine) as sess:
                    ...
                    if keywords:
                        ...
                        keyword_filters = [Tweet.full_text.ilike(f"%{k}%") for k in keyword_list]
                        twitter_query = twitter_query.where(or_(*keyword_filters))
                    
                    tweets = sess.exec(twitter_query.order_by(desc(Tweet.created_at)).limit(1000)).all()
                    ...
                    for tweet in tweets:
                        ...
                        if engagement >= min_engagement:
                            data.append({
                                ...
                                'url': f"https://twitter.com/user/status/{tweet.tweet_id}" if tweet.tweet_id else None
                            })
            ...
            if "Reddit" in sources:
                with Session(engine) as sess:
                    ...
                    if keywords:
                        keyword_list = [k.strip().lower() for k in keywords.split(",")]
                        keyword_filters = [RedditPost.title.ilike(f"%{k}%") for k in keyword_list]
                        reddit_query = reddit_query.where(or_(*keyword_filters))
                    
                    reddit_posts = sess.exec(reddit_query.order_by(desc(RedditPost.created_at)).limit(500)).all()
        ...
        for i, item in enumerate(current_data):
            with st.container():
                ...
                # Content
                st.markdown(f"**{item['content'][:200]}{'...' if len(item['content']) > 200 else ''}**")
    ...
    with col2:
        ...
        if show_charts and data:
            ...
            if len(data) > 1:
                ...
                daily_engagement = df.groupby('date_only')['engagement'].sum().reset_index()
        ...
        if data:
            ...
            st.metric("Avg Engagement", f"{np.mean([item['engagement'] for item in data]):.0f}")
            st.metric("Top Engagement", f"{max([item['engagement'] for item in data]):,}")
