...
# Page config
st.set_page_config(page_title="Top Items - Degen Digest", page_icon="🔥", layout="wide")
...
all_sources = list({item.get("source", "unknown") for item in items})
...
sort_by = st.sidebar.selectbox(
    "Sort by:", ["Engagement Score", "Likes", "Retweets", "Views", "Replies"], index=0
)
...
if filtered_items:
    st.sidebar.metric(
        "Top Engagement", f"{filtered_items[0].get('engagement_score', 0):.0f}"
    )
    st.sidebar.metric(
        "Total Likes", sum(item.get("likeCount", 0) for item in filtered_items)
    )
    ...
    if not filtered_items:
        ...
        for i, item in enumerate(filtered_items[:50]):  # Show top 50
            ...
            with st.expander(f"📊 Details for #{i+1}"):
                ...
                with col1:
                    st.metric(
                        "Engagement Score", f"{item.get('engagement_score', 0):.0f}"
                    )
