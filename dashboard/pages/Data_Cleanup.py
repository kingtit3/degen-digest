...
# Page config
st.set_page_config(
    page_title="Data Cleanup - Degen Digest",
    page_icon="🧹",
    layout="wide"
)
        ...
        if file_path.exists():
            try:
                ...
                if isinstance(data, list):
                    ...
                    count = sum(len(data.get(key, [])) for key in ['tweets', 'reddit_posts', 'telegram_messages', 'news_articles'])
                else:
                    count = 0
...
if st.sidebar.button("🔄 Run Data Deduplication"):
    with st.spinner("Running deduplication..."):
        import subprocess
        try:
            result = subprocess.run(["python", "scripts/deduplicate_data.py"], 
...
# Main content tabs
tab1, tab2, tab3, tab4 = st.tabs(["📊 Data Overview", "🔍 Deduplication Report", "📁 File Management", "⚙️ Cleanup Tools"])
    ...
    if original_stats:
        ...
        with col4:
            if deduplication_report:
                duplicates_removed = deduplication_report['summary']['total_duplicates_removed']
        ...
        with col2:
            fig = px.pie(df_sources, values='Items', names='Source', title="Distribution by Source")
            st.plotly_chart(fig, use_container_width=True)
...
with tab2:
    ...
    if deduplication_report:
        ...
        with col1:
            fig = go.Figure()
            fig.add_trace(go.Bar(name='Original', x=df_comparison['Source'], y=df_comparison['Original']))
            fig.add_trace(go.Bar(name='Final', x=df_comparison['Source'], y=df_comparison['Final']))
            fig.update_layout(title="Items Before vs After Deduplication", barmode='group')
            st.plotly_chart(fig, use_container_width=True)
        
        with col2:
            fig = px.bar(df_comparison, x='Source', y='Rate (%)', title="Deduplication Rate by Source")
            st.plotly_chart(fig, use_container_width=True)
        ...
        st.write(f"**Deduplication Date:** {deduplication_report['deduplication_date']}")
        
    else:
        st.info("No deduplication report found. Run the deduplication process to see results.")
    ...
    if original_stats and deduplicated_stats:
        ...
        for source in original_stats.keys():
            if source in deduplicated_stats:
                ...
                size_reduction = ((original_size - deduplicated_size) / original_size * 100) if original_size > 0 else 0
        ...
        if comparison_data:
            ...
            fig = px.bar(df_size_comparison, x='Source', y=['Original Size (MB)', 'Deduplicated Size (MB)'],
                        title="File Size Comparison", barmode='group')
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col1:
        ...
        if st.button("Backup Original Data"):
            with st.spinner("Creating backup..."):
                ...
                # Copy original files
                files_to_backup = [
                    "twitter_raw.json", "reddit_raw.json", "telegram_raw.json",
                    "newsapi_raw.json", "consolidated_data.json"
                ]
    ...
    with col1:
        if st.button("Export Deduplicated Data"):
            if deduplicated_stats:
                ...
                deduplicated_file = Path("output/deduplicated/consolidated_data_deduplicated.json")
