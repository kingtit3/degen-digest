...
class DataMerger:
    ...
    def generate_merge_report(self):
        ...
        if report['local_data'].get('Tweet', 0) < 100:
            report['recommendations'].append("Low tweet count - consider running Twitter scraper")
        
        if report['local_data'].get('RedditPost', 0) < 50:
            report['recommendations'].append("Low Reddit post count - consider running Reddit scraper")
        
        missing_files = [f for f, status in report['local_data'].items() if status == "Missing"]
        if missing_files:
