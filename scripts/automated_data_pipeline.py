...
class AutomatedDataPipeline:
    ...
    def __init__(self):
        ...
        self.last_twitter_run_file = self.project_root / "output" / "last_twitter_run.txt"
    ...
    def should_run_twitter(self) -> bool:
        ...
        try:
            with open(self.last_twitter_run_file, 'r') as f:
                last_run_str = f.read().strip()
                last_run = datetime.fromisoformat(last_run_str)
                
            time_since_last = datetime.now(timezone.utc) - last_run
            return time_since_last.total_seconds() >= (self.twitter_interval_hours * 3600)
    ...
    def run_continuous_pipeline(self):
        ...
        while True:
            try:
                ...
                logger.info(f"📅 Pipeline run at: {current_time.strftime('%Y-%m-%d %H:%M:%S UTC')}")
                ...
                if self.should_run_twitter():
                    scrapers_to_run.append(("scrapers/enhanced_twitter_scraper.py", "Enhanced Twitter Scraper"))
                    scrapers_to_run.append(("scrapers/twitter_apify.py", "Twitter Apify Scraper"))
                ...
                if self.run_scraper("run_intelligent_analysis.py", "Intelligent Analysis"):
                ...
                logger.info(f"✅ Pipeline run completed. {successful_scrapes}/{len(scrapers_to_run)} scrapers successful")
                logger.info("=" * 60)
    ...
    def check_data_quality(self):
        ...
        try:
            ...
            if db_file.exists():
                ...
                logger.info(f"🗄️ Recent items in DB: {recent_tweets} tweets, {recent_reddit} reddit posts")
                conn.close()
