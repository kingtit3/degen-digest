...
class EnhancedDataPipeline:
    ...
    async def collect_all_data(self):
        ...
        logger.info(f"Data collection completed. Total sources: {len(self.data_sources)}")
    ...
    async def load_existing_data(self):
        ...
        if twitter_file.exists():
            try:
                ...
                enhanced_twitter = self.enhance_twitter_data(twitter_data[:100])  # Limit to 100 items
                ...
                logger.info(f"Loaded {len(enhanced_twitter)} Twitter items from existing file")
        ...
        if reddit_file.exists():
            try:
                ...
                # Convert to enhanced format
                enhanced_reddit = self.enhance_reddit_data(reddit_data[:50])  # Limit to 50 items
                ...
                logger.info(f"Loaded {len(enhanced_reddit)} Reddit items from existing file")
        ...
        if telegram_file.exists():
            try:
                ...
                # Convert to enhanced format
                enhanced_telegram = self.enhance_telegram_data(telegram_data[:50])  # Limit to 50 items
                ...
                logger.info(f"Loaded {len(enhanced_telegram)} Telegram items from existing file")
    ...
    def enhance_twitter_data(self, twitter_data: List[Dict]) -> List[Dict]:
        ...
        for tweet in twitter_data:
            try:
                ...
                engagement_velocity = (likes + retweets * 2 + replies * 3 + quotes * 2) / 10
    ...
    def enhance_reddit_data(self, reddit_data: List[Dict]) -> List[Dict]:
        ...
        for post in reddit_data:
            try:
                ...
                created_at = datetime.fromisoformat(post.get('published', '').replace('Z', '+00:00'))
                hours_since_creation = (datetime.utcnow() - created_at).total_seconds() / 3600
                
                score = post.get('score', 0)
                comments = post.get('numComments', 0)
                
                engagement_velocity = (score + comments * 2) / max(hours_since_creation, 1)
    ...
    async def collect_discord_data(self):
        ...
        try:
            ...
            async with DiscordScraper() as scraper:
                discord_data = await scraper.scrape_discord_channels(max_messages=20)  # Reduced for speed
        ...
        for item in self.processed_data:
            try:
                ...
                predictions.append({
                    ...
                    'content_preview': item.get('text', '')[:100] + '...' if item.get('text') else '',
                    'timestamp': datetime.utcnow().isoformat()
                })
        ...
        for source, items in source_analysis.items():
            if items:
                trends[source] = {
                    ...
                    'avg_engagement_velocity': np.mean([i.get('engagement_velocity', 0) for i in items]),
                    'avg_viral_coefficient': np.mean([i.get('viral_coefficient', 0) for i in items]),
                    'top_categories': self.get_top_categories(items),
                    'viral_items': len([i for i in items if i.get('viral_prediction', {}).get('score', 0) > 0.5])
                }
    ...
    def get_top_categories(self, items: List[Dict], top_n: int = 5) -> List[Dict]:
        ...
        return [{'category': cat, 'count': count} for cat, count in sorted_categories[:top_n]]
    ...
    def generate_summary_report(self, output_dir: Path):
        ...
        report = {
            ...
            'viral_predictions_summary': {
                ...
                'high_viral_score': len([p for p in self.viral_predictions if p['prediction']['score'] > 0.7]),
                'medium_viral_score': len([p for p in self.viral_predictions if 0.3 < p['prediction']['score'] <= 0.7]),
                'low_viral_score': len([p for p in self.viral_predictions if p['prediction']['score'] <= 0.3])
            },
    ...
    def update_statistics(self, start_time: float):
        ...
        self.stats['viral_items_detected'] = len([p for p in self.viral_predictions if p['prediction']['score'] > 0.5])
...
async def main():
    ...
    try:
        ...
        if pipeline.viral_predictions:
            print("\nTop Viral Items:")
            sorted_predictions = sorted(pipeline.viral_predictions, 
                                     key=lambda x: x['prediction']['score'], 
                                     reverse=True)[:5]
            
            for i, pred in enumerate(sorted_predictions, 1):
                print(f"{i}. Score: {pred['prediction']['score']:.3f} | "
