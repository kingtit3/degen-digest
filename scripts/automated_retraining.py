...
class AutomatedRetraining:
    ...
    async def retrain_model(self):
        ...
        try:
            ...
            if len(pipeline.processed_data) < self.retraining_config["min_data_points"]:
                logger.warning(
                    f"Insufficient data for retraining. Need {self.retraining_config['min_data_points']}, got {len(pipeline.processed_data)}"
                )
                return
    ...
    def should_keep_new_model(self, new_performance: dict[str, float]) -> bool:
        ...
        if (
            new_performance.get("r2", 0)
            < self.retraining_config["performance_threshold"]
        ):
            logger.warning(
                f"New model performance below threshold: {new_performance.get('r2', 0)}"
            )
            return False
        ...
        if self.performance_history:
            ...
            performance_degradation = last_performance.get(
                "r2", 0
            ) - new_performance.get("r2", 0)

            if (
                performance_degradation
                > self.retraining_config["drift_detection_threshold"]
            ):
                logger.warning(
                    f"Performance degradation detected: {performance_degradation}"
                )
                return False
    ...
    def update_retraining_history(
        self, performance: dict[str, float], training_time: float
    ):
        ...
        history_entry = {
            ...
            "performance": performance,
            "training_time": training_time,
            "data_points": len(enhanced_predictor.model_performance)
            if enhanced_predictor.model_performance
            else 0,
        }
    ...
    def get_retraining_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "current_performance": enhanced_predictor.model_performance.get(
                "ensemble", {}
            ),
    ...
    def get_next_retrain_time(self) -> str:
        ...
        next_retrain = self.last_retrain_time + timedelta(
            hours=self.retraining_config["retrain_frequency_hours"]
        )
