...
class CloudStorageSync:
        ...
        if GCS_AVAILABLE:
            try:
                ...
                if not self.bucket.exists():
                    ...
                    logger.info(
                        f"Created bucket: {bucket_name} in project: {project_id}"
                    )
                else:
                    logger.info(
                        f"Using existing bucket: {bucket_name} in project: {project_id}"
                    )
            except Exception as e:
                logger.error(
                    f"Error accessing bucket {bucket_name} in project {project_id}: {e}"
                )
                self.bucket = None
        else:
            self.bucket = None
    ...
    def upload_all_data(self):
        ...
        pipeline_files = [
            (
                "enhanced_pipeline/viral_predictions.json",
                "enhanced_pipeline/viral_predictions.json",
            ),
            (
                "enhanced_pipeline/processed_data.json",
                "enhanced_pipeline/processed_data.json",
            ),
            (
                "enhanced_pipeline/summary_report.json",
                "enhanced_pipeline/summary_report.json",
            ),
            (
                "enhanced_pipeline/trends_analysis.json",
                "enhanced_pipeline/trends_analysis.json",
            ),
            (
                "enhanced_pipeline/pipeline_stats.json",
                "enhanced_pipeline/pipeline_stats.json",
            ),
        ]
    ...
    def get_file_metadata(self, cloud_path: str) -> dict[str, Any] | None:
    ...
    def restore_database(self, backup_timestamp: str):
        ...
        local_restore_path = (
            self.output_dir / f"degen_digest_restored_{backup_timestamp}.db"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync Degen Digest data with Google Cloud Storage"
    )
    parser.add_argument(
        "--direction",
    ...
    parser.add_argument(
        "--bucket", default="degen-digest-data", help="Cloud storage bucket name"
    )
    parser.add_argument(
        "--project", default="lucky-union-463615-t3", help="Google Cloud project ID"
    )
    parser.add_argument(
        "--list", action="store_true", help="List files in cloud storage"
    )
