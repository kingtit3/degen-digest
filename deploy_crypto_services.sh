...
# Deploy DexScreener service
echo "📦 Deploying DexScreener service..."
gcloud run deploy $DEXSCREENER_SERVICE \
    --image gcr.io/$PROJECT_ID/$DEXSCREENER_SERVICE \
    --platform managed \
    --region $REGION \
    --project $PROJECT_ID \
...
# Deploy DexPaprika service
echo "📦 Deploying DexPaprika service..."
gcloud run deploy $DEXPAPRIKA_SERVICE \
    --image gcr.io/$PROJECT_ID/$DEXPAPRIKA_SERVICE \
    --platform managed \
    --region $REGION \
    --project $PROJECT_ID \
    --allow-unauthenticated \
