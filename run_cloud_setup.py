...
def main():
    ...
    if not run_command(
        "pip list | grep google-cloud-storage", "Check Google Cloud Storage"
    ):
        print("⚠️  Google Cloud Storage not found. Installing...")
        if not run_command(
            "pip install google-cloud-storage", "Install Google Cloud Storage"
        ):
    ...
    if not run_command("gcloud auth list", "Check current authentication"):
        print(
            "⚠️  No authentication found. You may need to run: gcloud auth application-default login"
        )
    ...
    if not run_command(
        "gcloud config set project lucky-union-463615-t3", "Set project ID"
    ):
    ...
    if not run_command(
        "python scripts/cloud_storage_sync.py --direction upload", "Upload to cloud"
    ):
    ...
    if not run_command(
        "python scripts/cloud_storage_sync.py --list", "List cloud files"
    ):
    ...
    if not run_command(
        "python scripts/cloud_storage_sync.py --backup", "Create database backup"
    ):
