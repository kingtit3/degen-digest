...
class DataQualityMonitor:
    ...
    def monitor_data_quality(self, data: List[Dict], source: str) -> Dict[str, Any]:
        ...
        self.data_sources_status[source] = {
            ...
            'status': 'healthy' if overall_score >= 0.8 else 'degraded' if overall_score >= 0.6 else 'critical'
        }
    ...
    def calculate_completeness(self, data: List[Dict]) -> float:
        ...
        optional_fields = ['engagement_velocity', 'viral_coefficient', 'influence_score']
    ...
    def calculate_accuracy(self, data: List[Dict]) -> float:
        ...
        for item in data:
            ...
            timestamp = item.get('timestamp') or item.get('published') or item.get('created_at')
    ...
    def calculate_validity(self, data: List[Dict]) -> float:
        ...
        for item in data:
            ...
            if not item.get('text') and not item.get('title') and not item.get('summary'):
                is_valid = False
        ...
        for metric_name, threshold in self.quality_thresholds.items():
            if metric_name in metrics:
                ...
                if metric_value < threshold:
                    alerts.append(f"{metric_name} below threshold: {metric_value:.3f} < {threshold}")
